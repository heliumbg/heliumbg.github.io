...
notice=$wpf(notice);var msg_box=$wpf("#wpf-msg-box");msg_box.append(notice);msg_box.appendTo('body');msg_box.show(150);notice.delay(type==='error'?8000:4000).fadeOut(200,function(){$wpf(this).remove()})}
...
window.wpforo_prev_submit_time=0;wpforo_wrap.on('submit','form',function(){if(window.wpforo_prev_submit_time){if(Date.now()-window.wpforo_prev_submit_time<10000)return!1}else{var textareaid=$(this).data('textareaid');if(textareaid){var bodyminlength=$(this).data('bodyminlength');var bodymaxlength=$(this).data('bodymaxlength');if(bodyminlength||bodymaxlength){var body_stat=wpforo_editor.get_stats(textareaid);if(bodyminlength){if(body_stat.chars<bodyminlength&&!body_stat.embeds&&!body_stat.links&&!body_stat.imgs){wpforo_notice_show('Content characters length must be greater than %d','error',bodyminlength);return!1}}
...
wpforo_load_show();window.wpforo_prev_submit_time=Date.now();window.onbeforeunload=null;setTimeout(function(){window.wpforo_prev_submit_time=0;wpforo_load_hide()},10000)}});wpforo_wrap.on('click','.wpf-spoiler-head',function(){var spoiler_wrap=$(this).parents('.wpf-spoiler-wrap');if(spoiler_wrap.length){spoiler_wrap=$(spoiler_wrap[0]);if(!spoiler_wrap.hasClass('wpf-spoiler-processing')){spoiler_wrap.toggleClass("wpf-spoiler-open").addClass("wpf-spoiler-processing");var spoiler_body=$('.wpf-spoiler-body',spoiler_wrap);if(spoiler_body.length){var spoiler_chevron=$('.wpf-spoiler-chevron',spoiler_wrap);$(spoiler_chevron[0]).toggleClass('fa-chevron-down fa-chevron-up');$(spoiler_body[0]).slideToggle(500,function(){spoiler_wrap.removeClass("wpf-spoiler-processing");if(!spoiler_wrap.hasClass('wpf-spoiler-open')){$('.wpf-spoiler-wrap.wpf-spoiler-open .wpf-spoiler-head',spoiler_wrap).trigger("click")}})}}}});wpforo_wrap.on('click','#add_wpftopic:not(.not_reg_user)',function(){var form=$(".wpf-topic-create");var stat=form.is(":hidden");form.slideToggle("slow");wpforo_editor.set_content('');$('[name="topic[title]"]').trigger("focus");var add_wpftopic='<i class="fas fa-times" aria-hidden="true"></i>';if(!stat)add_wpftopic=$('input[type="submit"]',form).val();$(this).html(add_wpftopic);$('html, body').animate({scrollTop:($(this).offset().top-35)},415)});wpforo_wrap.on('click','.wpf-answer-button .wpf-button:not(.not_reg_user)',function(){$(this).closest('.wpf-bottom-bar').hide()});wpforo_wrap.on('click','.wpfl-4 .add_wpftopic:not(.not_reg_user)',function(){var wrap=$(this).parents('div.wpfl-4');var form_wrap=$(".wpf-topic-form-extra-wrap",wrap);var is_just_one_forum=form_wrap.data('is_just_one_forum');if(!is_just_one_forum)$('.wpf-topic-form-ajax-wrap').empty();var stat=form_wrap.is(":hidden");var btn=$(".wpfl-4 .add_wpftopic");btn.html(btn.data('phrase'));$(".wpf-topic-form-extra-wrap").slideUp("slow");var add_wpftopic;if(stat){add_wpftopic='<i class="fas fa-times" aria-hidden="true"></i>';form_wrap.slideDown("slow")}else{add_wpftopic=$(this).data('phrase');form_wrap.slideUp("slow")}
...
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpforo-unlike',function(){wpforo_load_show();var postid=$(this).data('postid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,likestatus:0,action:'wpforo_like_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
var like_count;like_count=that.children(".wpf-like-count").text();like_count=parseInt(like_count)-1;that.children(".wpf-like-count").text(like_count)}
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpforo-voteup',function(){wpforo_load_show();var type=$(this).data('type'),postid=$(this).data('postid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{itemtype:type,postid:postid,votestatus:'up',action:'wpforo_vote_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpforo-votedown',function(){wpforo_load_show();var type=$(this).data('type'),postid=$(this).data('postid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{itemtype:type,postid:postid,votestatus:'down',action:'wpforo_vote_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
if(response.stat===1){var wpfvote_num=that.parents('.post-wrap').find('.wpfvote-num'),count=wpfvote_num.text();wpfvote_num.text(--count).fadeIn()}
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpf-toggle-answer',function(){wpforo_load_show();var postid=$(this).data('postid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,answerstatus:0,action:'wpforo_answer_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpf-toggle-not-answer',function(){wpforo_load_show();var postid=$(this).data('postid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,answerstatus:1,action:'wpforo_answer_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpforo-quote',function(){wpforo_load_show();var main_form=$('form.wpforo-main-form[data-textareaid]');var wrap=main_form.closest('.wpf-form-wrapper');wrap.show();var post_wrap=$(this).closest('[id^=post-][data-postid]');var postid=post_wrap.data('postid');if(!postid)postid=0;$(".wpf-form-post-parentid").val(postid);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,action:'wpforo_quote_ajax'}}).done(function(response){var phrase=wpforo_phrase('Reply with quote');phrase=phrase.charAt(0).toUpperCase()+phrase.slice(1);$(".wpf-reply-form-title").html(phrase);$(".wpf-form-postid",main_form).val(0);wpforo_editor.set_content(response,wpforo_editor.get_main());$('html, body').animate({scrollTop:wrap.offset().top},1000);wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-report',function(){wpforo_load_show();var form=$("form#wpforo-report");$("#wpforo-report-postid",form).val($(this).data('postid'));wpforo_dialog_show('',form,'45%','295px');$("#wpforo-report-content",form).trigger("focus");wpforo_load_hide()});$(document).on('click','#wpforo-report-send',wpforo_report_send);$(document).on('keydown','form#wpforo-report',function(e){if((e.ctrlKey||e.metaKey)&&(e.code==='Enter'||e.code==='NumpadEnter')){wpforo_report_send()}});function wpforo_report_send(){wpforo_load_show();var postid=$('#wpforo-report-postid').val();var messagecontent=$('#wpforo-report-content').val();if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,reportmsg:messagecontent,action:'wpforo_report_ajax'}}).done(function(response){try{response=JSON.parse(response);wpforo_dialog_hide();$('#wpforo-report-content').val('');wpforo_load_hide();wpforo_notice_show(response)}catch(e){console.log(e)}})}}
wpforo_wrap.on('click','.wpforo-sticky',function(){wpforo_load_show();var topicid=$(this).data('topicid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{topicid:topicid,status:'sticky',action:'wpforo_sticky_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpforo-unsticky',function(){wpforo_load_show();var topicid=$(this).data('topicid'),that=$(this);if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{topicid:topicid,status:'unsticky',action:'wpforo_sticky_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
wpforo_load_hide();wpforo_notice_show(response.notice)})}});wpforo_wrap.on('click','.wpforo-approve',function(){wpforo_load_show();var status_value='approve';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfapprove","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_approve_ajax'}}).done(function(response){if(response!==0){$("#"+postid_value).removeClass('wpforo-approve').addClass('wpforo-unapprove');$("#approveicon"+postid).removeClass('fa-check').addClass('fa-exclamation-circle');$("#wpforo-wrap #post-"+postid+" .wpf-mod-message").hide();$("#wpforo-wrap .wpf-status-title").hide();$("#approvetext"+postid).text(' '+wpforo_phrase('Unapprove'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Unapprove'))}}
wpforo_load_hide();window.location.reload()})}});wpforo_wrap.on('click','.wpforo-unapprove',function(){wpforo_load_show();var status_value='unapprove';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfapprove","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_approve_ajax'}}).done(function(response){if(parseInt(response)!==0){$("#"+postid_value).removeClass('wpforo-unapprove').addClass('wpforo-approve');$("#approveicon"+postid).removeClass('fa-exclamation-circle').addClass('fa-check');$('#wpforo-wrap #post-'+postid+' .wpf-mod-message').visible();$('#wpforo-wrap .wpf-status-title').visible();$("#approvetext"+postid).text(' '+wpforo_phrase('Approve'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Approve'))}}
wpforo_load_hide();window.location.reload()})}});wpforo_wrap.on('click','.wpforo-private',function(){wpforo_load_show();var status_value='private';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfprivate","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_private_ajax'}}).done(function(response){if(parseInt(response)!==0){$("#"+postid_value).removeClass('wpforo-private').addClass('wpforo-public');$("#privateicon"+postid).removeClass('fa-eye-slash').addClass('fa-eye');$("#privatetext"+postid).text(' '+wpforo_phrase('Public'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Public'))}}
wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-public',function(){wpforo_load_show();var status_value='public';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfprivate","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_private_ajax'}}).done(function(response){if(parseInt(response)!==0){$("#"+postid_value).removeClass('wpforo-public').addClass('wpforo-private');$("#privateicon"+postid).removeClass('fa-eye').addClass('fa-eye-slash');$("#privatetext"+postid).text(' '+wpforo_phrase('Private'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Private'))}}
wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-solved',function(){wpforo_load_show();var status_value='solved';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfsolved","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_solved_ajax'}}).done(function(response){if(parseInt(response)!==0){$("#"+postid_value).removeClass('wpforo-solved').addClass('wpforo-unsolved');$("#solvedtext"+postid).text(' '+wpforo_phrase('Unsolved'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Unsolved'))}}
wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-unsolved',function(){wpforo_load_show();var status_value='unsolved';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfsolved","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_solved_ajax'}}).done(function(response){if(parseInt(response)!==0){$("#"+postid_value).removeClass('wpforo-unsolved').addClass('wpforo-solved');$("#solvedtext"+postid).text(' '+wpforo_phrase('Solved'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Solved'))}}
wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-close',function(){wpforo_load_show();var status_value='close';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfclose","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_close_ajax'}}).done(function(response){if(parseInt(response)!==0){$("#"+postid_value).removeClass('wpforo-close').addClass('wpforo-open');$("#closeicon"+postid).removeClass('fa-lock').addClass('fa-unlock');$("#closetext"+postid).text(' '+wpforo_phrase('Open'));if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Open'))}
...
wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-open',function(){wpforo_load_show();var status_value='closed';var postid_value=$(this).attr('id'),that=$(this);var postid=postid_value.replace("wpfclose","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:postid,status:status_value,action:'wpforo_close_ajax'}}).done(function(response){if(parseInt(response)!==0){if(that.is("[wpf-tooltip]")){that.attr("wpf-tooltip",wpforo_phrase('Close'))}
window.location.assign(response)}
wpforo_load_hide()})}});wpforo_wrap.on('click','.wpforo-edit',function(){var wrap=$(this).closest('[id^=post-][data-postid]');if(wrap.length&&$.active===0){var children=wrap.contents().not(":hidden");var loading=$('<div class="wpforo-loading-portable"><i class="fas fa-3x fa-circle-notch fa-spin"></i></div>');wrap.append(loading);var show_form=function(form,init){loading.remove();children.fadeOut('fast');var f=$(form);f.on('click','.wpf-edit-post-cancel',function(){f.fadeOut('fast');children.fadeIn('slow');$('html, body').animate({scrollTop:(wrap.offset().top-80)},415)});if(init){f.appendTo(wrap);wpforo_trigger_custom_event(document,'wpforo_topic_portable_form',f)}
f.fadeIn('slow');$('html, body').animate({scrollTop:(wrap.offset().top-80)},415)}
var f=$('.wpf-form-wrapper',wrap);if(f.length){show_form(f)}else{$.ajax({type:'POST',url:wpforo.ajax_url,data:{postid:parseInt(wrap.data('postid')),action:'wpforo_post_edit'}}).done(function(response){try{response=JSON.parse(response);if(parseInt(response.state))show_form(response.html,!0)}catch(e){console.log(e)}})}}});wpforo_wrap.on('click','.wpforo-delete',function(){wpforo_load_show();var ok=confirm(wpforo_ucwords(wpforo_phrase('are you sure you want to delete?')));if(ok&&$.active===0){var postid_value=$(this).attr('id');var is_topic=postid_value.indexOf("topic");var postid,status_value;if(is_topic===-1){postid=postid_value.replace("wpfreplydelete","");status_value='reply'}else{postid=postid_value.replace("wpftopicdelete","");status_value='topic'}
var forumid=0;var wpf_forumid=$("input[type='hidden'].wpf-form-forumid");if(wpf_forumid.length)forumid=wpf_forumid.val();$.ajax({type:'POST',url:wpforo.ajax_url,data:{forumid:forumid,postid:postid,status:status_value,action:'wpforo_delete_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
...
var postid_value=$(this).attr('id');var itemid=postid_value.replace("wpfsubscribe-","");if($.active===0){$.ajax({type:'POST',url:wpforo.ajax_url,data:{itemid:itemid,type:type,status:status,action:'wpforo_subscribe_ajax'}}).done(function(response){try{response=JSON.parse(response)}catch(e){console.log(e)}
if(parseInt(response.stat)===1){$("#wpfsubscribe-"+itemid).removeClass('wpf-subscribe-'+type).addClass('wpf-unsubscribe-'+type).text(' '+wpforo_phrase('Unsubscribe'));wpforo_load_hide()}else{wpforo_load_hide()}
