...
window.fixMenuLogoPosition=fixMenuLogoPosition})();(function($){'use strict';var isVerticalMenu=$('.header-main').hasClass('header-layout-vertical'),isHamburgerMenu=$('.header-main').hasClass('header-layout-fullwidth_hamburger');$(window).resize(function(){window.updateCTClientSize(!1);window.updateCTInnerSize()});window.menuResizeTimeoutHandler=!1;var megaMenuSettings={};function getOffset(elem){if(elem.getBoundingClientRect&&window.ctBrowser.platform.name!='ios'){var bound=elem.getBoundingClientRect(),html=elem.ownerDocument.documentElement,htmlScroll=getScroll(html),elemScrolls=getScrolls(elem),isFixed=(styleString(elem,'position')=='fixed');return{x:parseInt(bound.left)+elemScrolls.x+((isFixed)?0:htmlScroll.x)-html.clientLeft,y:parseInt(bound.top)+elemScrolls.y+((isFixed)?0:htmlScroll.y)-html.clientTop}}
var element=elem,position={x:0,y:0};if(isBody(elem))return position;while(element&&!isBody(element)){position.x+=element.offsetLeft;position.y+=element.offsetTop;if(window.ctBrowser.name=='firefox'){if(!borderBox(element)){position.x+=leftBorder(element);position.y+=topBorder(element)}
var parent=element.parentNode;if(parent&&styleString(parent,'overflow')!='visible'){position.x+=leftBorder(parent);position.y+=topBorder(parent)}}else if(element!=elem&&window.ctBrowser.name=='safari'){position.x+=leftBorder(element);position.y+=topBorder(element)}
element=element.offsetParent}
if(window.ctBrowser.name=='firefox'&&!borderBox(elem)){position.x-=leftBorder(elem);position.y-=topBorder(elem)}
...
$item.css(css)})}
function isResponsiveMenuVisible(){if(window.ctSettings.tabletLandscape){return window.ctOptions.clientWidth<1213}
if(window.ctSettings.tabletPortrait){return window.ctOptions.clientWidth<980}
return window.ctOptions.clientWidth<768}
window.isResponsiveMenuVisible=isResponsiveMenuVisible;function isTopAreaVisible(){return window.ctSettings.topAreaMobileDisable?window.ctOptions.clientWidth>=768:!0}
window.isTopAreaVisible=isTopAreaVisible;function isVerticalToggleVisible(){return window.ctOptions.clientWidth>1600}
...
if(isVerticalMenu){var container_width=window.ctOptions.clientWidth-$('#site-header-wrapper').outerWidth()}else if(isHamburgerMenu){var container_width=window.ctOptions.clientWidth-$('#primary-menu').outerWidth()}else{var $container=$item.closest('.header-main'),container_width=$container.width(),container_padding_left=parseInt($container.css('padding-left')),container_padding_right=parseInt($container.css('padding-right')),parent_width=$item.parent().outerWidth()}
...
if($('.header-main').first().hasClass('header-style-vertical')){if($item.offset().top-$('#site-header').offset().top+$item.outerHeight()>$(window).height()){$item.addClass('vertical-invert');$item.css({top:-($item.offset().top-$('#site-header').offset().top+$item.outerHeight()-$(window).height())+'px'})}}else if($item,$item.parents('#primary-menu ul').length){if($item.offset().top-$('#site-header').offset().top+$item.outerHeight()>$(window).height()){$item.addClass('vertical-invert');$item.css({top:-($item.offset().top-$('#site-header').offset().top+$item.outerHeight()-$(window).height())+'px'})}}}})},50);$('#primary-navigation').removeClass('without-transition')}}
$('#primary-navigation .submenu-languages').addClass('dl-submenu');$('#primary-navigation > ul> li.menu-item-language').addClass('menu-item-parent');fixMegaMenuWithSettings();$('#primary-navigation').dlmenu({animationClasses:{classin:'dl-animate-in',classout:'dl-animate-out',backLabel:ct_dlmenu_settings.backLabel,showCurrentLabel:ct_dlmenu_settings.showCurrentLabel},onLevelClick:function(el,name){$('html, body').animate({scrollTop:0})}});primary_menu_reinit();$('.hamburger-toggle').click(function(e){e.preventDefault();$(this).closest('#primary-navigation').toggleClass('hamburger-active');$('.hamburger-overlay').toggleClass('active')});$('.vertical-toggle').click(function(e){e.preventDefault();$(this).closest('#site-header-wrapper').toggleClass('vertical-active')});$(function(){$(window).resize(function(){if(window.menuResizeTimeoutHandler){clearTimeout(window.menuResizeTimeoutHandler)}
window.menuResizeTimeoutHandler=setTimeout(primary_menu_reinit,50)})});$('#primary-navigation a').click(function(e){var $item=$(this);if($('#primary-menu').hasClass('no-responsive')&&window.ctSettings.isTouch&&$item.next('ul').length){e.preventDefault()}})})(jQuery);
...
(function($){'use strict';function HeaderAnimation(el,options){this.el=el;this.$el=$(el);this.options={startTop:1};$.extend(this.options,options);this.initialize()}
HeaderAnimation.prototype={initialize:function(){var self=this;this.$wrapper=$('#site-header-wrapper');this.$topArea=$('#top-area');this.topAreaInSiteHeader=$('#site-header #top-area').length>0;this.$headerMain=$('.header-main',this.$el);this.hasAdminBar=document.body.className.indexOf('admin-bar')!=-1;this.htmlOffset=this.hasAdminBar?parseInt($('html').css('margin-top')):0;this.scrollTop=0;this.topOffset=0;this.settedWrapperHeight=!1;this.initedForDesktop=!1;this.hideWrapper=this.$wrapper.hasClass('site-header-wrapper-transparent');this.videoBackground=$('.page-title-block .ct-video-background').length&&$('.page-title-block .ct-video-background').data('headerup');if(this.$el.hasClass('header-on-slideshow')&&$('#main-content > *').first().is('.ct-slideshow, .block-slideshow')){this.$wrapper.css({position:'absolute'})}
if(this.$el.hasClass('header-on-slideshow')&&$('#main-content > *').first().is('.ct-slideshow, .block-slideshow')){this.$wrapper.addClass('header-on-slideshow')}else{this.$el.removeClass('header-on-slideshow')}
...
if(this.getScrollY()>=this.options.startTop){if(!this.$el.hasClass('shrink')){var shrinkClass='shrink fixed';if(window.ctSettings.fillTopArea){shrinkClass+=' fill'}
...
var i=this;return o._=t,this.on(e,o,n)},emit:function(e){var t=[].slice.call(arguments,1),n=((this.e||(this.e={}))[e]||[]).slice(),o=0,i=n.length;for(o;o<i;o++)n[o].fn.apply(n[o].ctx,t);return this},off:function(e,t){var n=this.e||(this.e={}),o=n[e],i=[];if(o&&t)for(var r=0,a=o.length;r<a;r++)o[r].fn!==t&&o[r].fn._!==t&&i.push(o[r]);return i.length?n[e]=i:delete n[e],this}},t.exports=o},{}],7:[function(t,n,o){!function(i,r){if("function"==typeof e&&e.amd)e(["module","select"],r);else if("undefined"!=typeof o)r(n,t("select"));else{var a={exports:{}};r(a,i.select),i.clipboardAction=a.exports}}(this,function(e,t){"use strict";function n(e){return e&&e.__esModule?e:{default:e}}
...
return c(t,e),d(t,[{key:"resolveOptions",value:function e(){var t=arguments.length>0&&void 0!==arguments[0]?arguments[0]:{};this.action="function"==typeof t.action?t.action:this.defaultAction,this.target="function"==typeof t.target?t.target:this.defaultTarget,this.text="function"==typeof t.text?t.text:this.defaultText}},{key:"listenClick",value:function e(t){var n=this;this.listener=(0,f.default)(t,"click",function(e){return n.onClick(e)})}},{key:"onClick",value:function e(t){var n=t.delegateTarget||t.currentTarget;this.clipboardAction&&(this.clipboardAction=null),this.clipboardAction=new u.default({action:this.action(n),target:this.target(n),text:this.text(n),trigger:n,emitter:this})}},{key:"defaultAction",value:function e(t){return l("action",t)}},{key:"defaultTarget",value:function e(t){var n=l("target",t);if(n)return document.querySelector(n)}},{key:"defaultText",value:function e(t){return l("text",t)}},{key:"destroy",value:function e(){this.listener.destroy(),this.clipboardAction&&(this.clipboardAction.destroy(),this.clipboardAction=null)}}],[{key:"isSupported",value:function e(){var t=arguments.length>0&&void 0!==arguments[0]?arguments[0]:["copy","cut"],n="string"==typeof t?[t]:t,o=!!document.queryCommandSupported;return n.forEach(function(e){o=o&&!!document.queryCommandSupported(e)}),o}}]),t}(s.default);e.exports=h})},{"./clipboard-action":7,"good-listener":4,"tiny-emitter":6}]},{},[8])(8)});(function($){if(document.getElementById("clip-link")){new Clipboard('.btn-clipboard')}})(jQuery);(function($){$(function(){$('.fancybox-popup a').each(function(){$(this).fancybox({title:!1,wrapCSS:'custom-popup',width:600,autoResize:!1,autoSize:!1,autoHeight:!0})})})})(jQuery);
(function($){'use strict';var lazy_effects={'clip':{show:function(element){$(element.el).animate({transform:'scale(1.1)',},{duration:200,}).animate({transform:'scale(1)',},{duration:200,complete:function(){if(element.options.queueType=='sync')
...
function getOffset(elem){if(elem.getBoundingClientRect&&window.ctBrowser.platform.name!='ios'){var bound=elem.getBoundingClientRect(),html=elem.ownerDocument.documentElement,htmlScroll=getScroll(html),elemScrolls=getScrolls(elem),isFixed=(styleString(elem,'position')=='fixed');return{x:parseInt(bound.left)+elemScrolls.x+((isFixed)?0:htmlScroll.x)-html.clientLeft,y:parseInt(bound.top)+elemScrolls.y+((isFixed)?0:htmlScroll.y)-html.clientTop}}
var element=elem,position={x:0,y:0};if(isBody(elem))return position;while(element&&!isBody(element)){position.x+=element.offsetLeft;position.y+=element.offsetTop;if(window.ctBrowser.name=='firefox'){if(!borderBox(element)){position.x+=leftBorder(element);position.y+=topBorder(element)}
var parent=element.parentNode;if(parent&&styleString(parent,'overflow')!='visible'){position.x+=leftBorder(parent);position.y+=topBorder(parent)}}else if(element!=elem&&window.ctBrowser.name=='safari'){position.x+=leftBorder(element);position.y+=topBorder(element)}
element=element.offsetParent}
if(window.ctBrowser.name=='firefox'&&!borderBox(elem)){position.x-=leftBorder(elem);position.y-=topBorder(elem)}
...
var element=new Element(this,{effect:effect,group:group,delay:el_delay});elements.push(element)});if(elements.length>0){group.setElements(elements);self.groups.push(group)}})},getEffect:function(element){return $(element).data('ll-effect')||''},finishAnimation:function(){}};Group.prototype={is_visible:function(window_bottom){if(this.options.force)
...
(function($){'use strict';function getScrollY(elem){return window.pageYOffset||document.documentElement.scrollTop}
...
(function($){"use strict";$.fn.ctPreloader=function(callback){$(this).each(function(){var $el=$(this);if(!$el.prev('.preloader').length){$('<div class="preloader">').insertBefore($el)}
$el.data('ctPreloader',$('img, iframe',$el).add($el.filter('img, iframe')).length);if($el.data('ctPreloader')==0){$el.prev('.preloader').remove();callback();$el.trigger('ct-preloader-loaded');return}
$('img, iframe',$el).add($el.filter('img, iframe')).each(function(){var $obj=$('<img>');if($(this).prop('tagName').toLowerCase()=='iframe'){$obj=$(this)}
$obj.attr('src',$(this).attr('src'));$obj.on('load error',function(){$el.data('ctPreloader',$el.data('ctPreloader')-1);if($el.data('ctPreloader')==0){$el.prev('.preloader').remove();callback();$el.trigger('ct-preloader-loaded')}})})})}})(jQuery);(function($){"use strict";var oWidth=$.fn.width;$.fn.width=function(argument){if(arguments.length==0&&this.length==1&&this[0]===window){if(window.ctOptions.innerWidth!=-1){return window.ctOptions.innerWidth}
var width=oWidth.apply(this,arguments);window.updateCTInnerSize(width);return width}
return oWidth.apply(this,arguments)};var $page=$('#page');$(window).load(function(){var $preloader=$('#page-preloader');if($preloader.length&&!$preloader.hasClass('preloader-loaded')){$preloader.addClass('preloader-loaded')}});$('#site-header.animated-header').headerAnimation();if(!window.ctSettings.lasyDisabled){$('.wpb_text_column.wpb_animate_when_almost_visible.wpb_fade').each(function(){$(this).wrap('<div class="lazy-loading"></div>').addClass('lazy-loading-item').data('ll-effect','fading')});$('.ct-list.lazy-loading').each(function(){$(this).data('ll-item-delay','200');$('li',this).addClass('lazy-loading-item').data('ll-effect','slide-right')});$.lazyLoading()}
$.fn.updateTabs=function(){jQuery('.ct-tabs',this).each(function(index){var $tabs=$(this);$tabs.ctPreloader(function(){$tabs.easyResponsiveTabs({type:'default',width:'auto',fit:!1,activate:function(currentTab,e){var $tab=$(currentTab.target);var controls=$tab.attr('aria-controls');$tab.closest('.ui-tabs').find('.ct_tab[aria-labelledby="'+controls+'"]').trigger('tab-update')}})})});jQuery('.ct-tour',this).each(function(index){var $tabs=$(this);$tabs.ctPreloader(function(){$tabs.easyResponsiveTabs({type:'vertical',width:'auto',fit:!1,activate:function(currentTab,e){var $tab=$(currentTab.target);var controls=$tab.attr('aria-controls');$tab.closest('.ui-tabs').find('.ct_tab[aria-labelledby="'+controls+'"]').trigger('tab-update')}})})})};function fullwidth_block_after_update($item){$item.trigger('updateTestimonialsCarousel');$item.trigger('updateClientsCarousel');$item.trigger('fullwidthUpdate');$('.marker-wave svg',$item).hide();setTimeout(function(){$('.marker-wave svg',$item).show()},50)}
...
var offsetKey=window.ctSettings.isRTL?'right':'left';var cssData={width:pageWidth};cssData[offsetKey]=pageOffset.left-($prevElement.length?$prevElement.offset().left:0)+parseInt(pagePaddingLeft)-extra_padding;$item.css(cssData);if(!skipTrigger){fullwidth_block_after_update($item)}}
...
if(!window.disableCTSlideshowPreloaderHandle){jQuery('.ct-slideshow').each(function(){var $slideshow=$(this);$slideshow.ctPreloader(function(){})})}
$(function(){$('#ct-icons-loading-hide').remove();$('#ct-preloader-inline-css').remove();jQuery('iframe').not('.ct-video-background iframe').each(function(){$(this).ctPreloader(function(){})});jQuery('.ct-video-background').each(function(){var $videoBG=$(this);var $videoContainer=$('.ct-video-background-inner',this);var ratio=$videoBG.data('aspect-ratio')?$videoBG.data('aspect-ratio'):'16:9';var regexp=/(\d+):(\d+)/;var $fullwidth=$videoBG.closest('.fullwidth-block');ratio=regexp.exec(ratio);if(!ratio||parseInt(ratio[1])==0||parseInt(ratio[2])==0){ratio=16/9}else{ratio=parseInt(ratio[1])/parseInt(ratio[2])}
function ctVideoUpdate(){$videoContainer.removeAttr('style');if($videoContainer.width()/$videoContainer.height()>ratio){$videoContainer.css({height:($videoContainer.width()/ratio)+'px',marginTop:-($videoContainer.width()/ratio-$videoBG.height())/2+'px'})}else{$videoContainer.css({width:($videoContainer.height()*ratio)+'px',marginLeft:-($videoContainer.height()*ratio-$videoBG.width())/2+'px'})}}
if($videoBG.closest('.page-title-block').length>0){ctVideoUpdate()}
if($fullwidth.length){$fullwidth.on('fullwidthUpdate',ctVideoUpdate)}else{$(window).resize(ctVideoUpdate)}});update_fullwidths(!1,!0);if(!window.ctSettings.parallaxDisabled){$('.fullwidth-block').each(function(){var $item=$(this),mobile_enabled=$item.data('mobile-parallax-enable')||'0';if(!window.ctSettings.isTouch||mobile_enabled=='1'){if($item.hasClass('fullwidth-block-parallax-vertical')){$('.fullwidth-block-background',$item).parallaxVertical('50%')}else if($item.hasClass('fullwidth-block-parallax-horizontal')){$('.fullwidth-block-background',$item).parallaxHorizontal()}}else{$('.fullwidth-block-background',$item).css({backgroundAttachment:'scroll'})}})}
$(window).resize(function(){update_fullwidths(!1,!1)});jQuery('select.ct-combobox, .ct-combobox select, .widget_archive select, .widget_product_categories select, .widget_layered_nav select, .widget_categories select').each(function(index){$(this).combobox()});jQuery('input.ct-checkbox, .ct-checkbox input').checkbox();if(typeof($.fn.ReStable)=="function"){jQuery('.ct-table-responsive').each(function(index){$('> table',this).ReStable({maxWidth:768,rowHeaders:$(this).hasClass('row-headers')})})}
jQuery('.fancybox').each(function(){$(this).fancybox()});function init_odometer(el){if(jQuery('.ct-counter-odometer',el).size()==0)
return;var odometer=jQuery('.ct-counter-odometer',el).get(0);var format=jQuery(el).closest('.ct-counter-box').data('number-format');format=format?format:'(ddd).ddd';var od=new Odometer({el:odometer,value:$(odometer).text(),format:format});od.update($(odometer).data('to'))}
window.ct_init_odometer=init_odometer;jQuery('.ct-counter').each(function(index){if(jQuery(this).closest('.ct-counter-box').size()>0&&jQuery(this).closest('.ct-counter-box').hasClass('lazy-loading')&&!window.ctSettings.lasyDisabled){jQuery(this).addClass('lazy-loading-item').data('ll-effect','action').data('item-delay','0').data('ll-action-func','ct_init_odometer');jQuery('.ct-icon',this).addClass('lazy-loading-item').data('ll-effect','fading').data('item-delay','0');jQuery('.ct-counter-text',this).addClass('lazy-loading-item').data('ll-effect','fading').data('item-delay','0');return}
...
$(window).load(function(){if(window.location.href==$anhor.attr('href')){$anhor.click()}})});$('body').on('click','.post-footer-sharing .ct-button',function(e){e.preventDefault();$(this).closest('.post-footer-sharing').find('.sharing-popup').toggleClass('active')});$(window).scroll(function(){if(getScrollY()>0){$('.scroll-top-button').addClass('visible')}else{$('.scroll-top-button').removeClass('visible')}}).scroll();function getScrollY(elem){return window.pageYOffset||document.documentElement.scrollTop}
$('a.hidden-email').each(function(){$(this).attr('href','mailto:'+$(this).data('name')+'@'+$(this).data('domain'))});$('#colophon .footer-widget-area').ctPreloader(function(){$('#colophon .footer-widget-area').isotope({itemSelector:'.widget',layoutMode:'masonry'})});$('body').updateTabs()});$(document).on('show.vc.accordion','[data-vc-accordion]',function(){var $target=$(this).data('vc.accordion').getContainer();var correction=0;if($target.find('.vc_tta-tabs').length&&!$(this).is(':visible'))return;if($('#site-header.animated-header').length&&$('#site-header').hasClass('fixed')){var shrink=$('#site-header').hasClass('shrink');$('#site-header').addClass('scroll-counting');$('#site-header').addClass('fixed shrink');correction=$('#site-header').outerHeight()+$('#site-header').position().top;if(!shrink){$('#site-header').removeClass('fixed shrink')}
$('#site-header').removeClass('scroll-counting')}
var target_top=$target.offset().top-correction-100+1;$('html, body').stop(!0,!0).animate({scrollTop:target_top},500,'easeInOutQuint')});var vc_update_fullwidth_init=!0;$(document).on('vc-full-width-row',function(e){if(window.ctOptions.clientWidth-$page.width()>25||window.ctSettings.isRTL){for(var i=1;i<arguments.length;i++){var $el=$(arguments[i]);$el.addClass("vc_hidden");var $el_full=$el.next(".vc_row-full-width");$el_full.length||($el_full=$el.parent().next(".vc_row-full-width"));var el_margin_left=parseInt($el.css("margin-left"),10),el_margin_right=parseInt($el.css("margin-right"),10),offset=0-$el_full.offset().left-el_margin_left+$('#page').offset().left+parseInt($('#page').css('padding-left')),width=$('#page').width();var offsetKey=window.ctSettings.isRTL?'right':'left';var cssData={position:"relative",left:offset,"box-sizing":"border-box",width:$("#page").width()};cssData[offsetKey]=offset;if($el.css(cssData),!$el.data("vcStretchContent")){var padding=-1*offset;0>padding&&(padding=0);var paddingRight=width-padding-$el_full.width()+el_margin_left+el_margin_right;0>paddingRight&&(paddingRight=0),$el.css({"padding-left":padding+"px","padding-right":paddingRight+"px"})}
$el.attr("data-vc-full-width-init","true"),$el.removeClass("vc_hidden")}}
update_fullwidths(!0,vc_update_fullwidth_init);vc_update_fullwidth_init=!1})})(jQuery);(function($){"use strict";$('.menu-item-search a').on('click',function(e){e.preventDefault();$('.menu-item-search').toggleClass('active')})})(jQuery);
...
(function($){'use strict';$(function(){$('body').updateAccordions()});$.fn.updateAccordions=function(){$('.ct_accordion',this).each(function(index){var $accordion=$(this);$accordion.ctPreloader(function(){var $tabs,interval=$accordion.attr("data-interval"),active_tab=!isNaN($accordion.data('active-tab'))&&parseInt($accordion.data('active-tab'))>0?parseInt($accordion.data('active-tab'))-1:!1,collapsible=$accordion.data('collapsible')==='yes';$tabs=$accordion.find('.ct_accordion_wrapper').accordion({header:"> div > .ct_accordion_header",autoHeight:!1,heightStyle:"content",active:active_tab,collapsible:collapsible,navigation:!0,activate:function(event,ui){if(ui.newPanel.size()>0){ui.newPanel.trigger('accordion-update')}},beforeActivate:function(event,ui){if(ui.newPanel.size()>0){$("html, body").animate({scrollTop:ui.newPanel.closest('.ct_accordion').offset().top-200},300)}}})})})}})(jQuery);
